...
本稿では, @BlanchardKahn1980 および @Klein2000 の決定論バージョンを考察しよう.\footnote{@Klein2000 はQZ分解を用いた解法を提案している. 理論的にはワイエルシュトラス標準形を使う方が綺麗な結果が得られるので,
...
```{theorem}
\label{thm:blockinv}$A,B,C,D$ は適当な次元の行列であるとする. $A$ および $D-CA^{-1}B$
...
%
```{corollary}
\label{cor:blockcor}$A,C,D$ は適当な次元の行列であるとする. $A$ および $D$ を正則とする.
...
によって定義する. 次の補題を証明しよう. 
```{lemma}
\label{lem:vecformula}$A$, $B$, $C$ を適当なサイズの行列とする. このとき, $\mathrm{vec}(ABC)=(C^{\top}\otimes A)\mathrm{vec}(B)$
が成り立つ. 
```
...
```{theorem, name='離散時間シルベスタ方程式'}
\label{thm:sylv}$A$, $B$, C を適当なサイズの行列, $I-\left(B^{\top}\otimes A\right)$
...
$$
x_{t+1}=Ax_{t}+Bu_{t},\quad t=0,1,\dots,\label{eq:sys}
$$
...
について
$$
\lim_{T\to\infty}\rho^{T}\|x_{T}\|=0\label{eq:tvc}
$$
が成り立つ $x$ を実行可能な解であるとする. 式 \eqref{eq:sys} を式 \eqref{eq:12} に適合するようにブロック行列表現すると
...
C_{u}
\end{bmatrix}u_{t}\label{eq:jordan}
$$
を得る. ただし, 
$$
...
やや天下り的ではあるが, 式\eqref{eq:jordan} 下段のシステムを, $t$ 期から $T$ 期までの情報を使って後ろ向きに
(forward-looking に) 解いてみよう.\footnote{経済学で forward というと「未来から現在に向かって」という意味で使われるようである. すなわち, 観察している情報が未来（前方）にあることを意識している.
...
y_{t}^{u} & =J_{u}^{-1}y_{t+1}^{u}-J_{u}^{-1}C_{u}u_{t}\nonumber \\
 ...
 & =\cdots\nonumber \\
 & =J_{u}^{t-T}y_{T}^{u}-\sum_{k=0}^{T-t-1}J_{u}^{-k-1}C_{u}u_{t+k}.\label{eq:yt}
\end{align}
ここで, $J_{u}^{-1}$ は安定行列であり, $T\to\infty$ のとき幾何数列のオーダーで減衰する. 一方 $y_{T}^{u}$
は $x_{T}$ の1次結合であるから, 仮定\eqref{eq:tvc}により$\lim_{T\to\infty}\|J_{u}^{t-T}y_{T}^{u}\|=0$.
つまり, 
$$
y_{t}^{u}=-\sum_{k=0}^{\infty}J_{u}^{-k-1}C_{u}u_{t+k}\label{eq:yu}
$$
が成り立つ.\footnote{$u_{t+k}$ の極限での振る舞いにも仮定を置かなければこの級数が収束しないかもしれない. 簡単のため級数の収束性を保証する条件が置かれているものとしよう. } 
...
\end{bmatrix}\begin{bmatrix}y_{t}^{s}\\
y_{t}^{u}
\end{bmatrix}\label{eq:cross}
$$
が成り立つ. $t=0$ したとき, \eqref{eq:cross} に現れる変数の内 $x_{0}^{1}=\bar{x}_{0}^{1}$
...
\begin{align}
\bar{x}_{0}^{1} & =P_{1s}y_{0}^{s}+P_{1u}\bar{y}_{0}^{u}\label{eq:7}\\
x_{0}^{2} & =P_{2s}y_{0}^{s}+P_{2u}\bar{y}_{0}^{u}\label{eq:9}
\end{align}
...
$$
が成り立つとすれば, 方程式 \eqref{eq:det0} の係数行列 $\Pi_{1}\in\mathbb{R}^{n\times(n_{s}+n_{2})}$
は横長の行列である. したがって, 方程式 \eqref{eq:det0} を満たす $(y_{0}^{s},x_{0}^{2})$
は無数に存在する. 解軌道が無数に存在するので解は\textbf{不決定 (indeterminate) }であるという. 一般に,
...
n_{s}+n_{2}<n=n_{1}+n_{2}\Longleftrightarrow n_{s}<n_{1}
$$
が成り立つとしよう. このとき, 係数行列 $\Pi_{1}$ は縦長の行列であるから, 方程式 \eqref{eq:det0}
...
\end{bmatrix}+Bu_{t}.\label{eq:recur}
\end{align}
$y_{t}^{u}$ は式\eqref{eq:yu} によって決定していることに注意せよ. なお, 式\eqref{eq:recur}
...
式\eqref{eq:recur} を使って, $x_{t}^{1}$ を消去する. $x_{t}^{1}$ の係数行列をそろえて
...
定理としてまとめておこう. 
```{theorem}
\label{thm:blkn}$n_{1}=n_{s}$ かつ $\det P_{1s}\neq0$ とする. このとき,
\eqref{eq:sys} の解は一意に定まり, $t=0,1,\dots$ について $x_{t}$ は次式で与えられる.
$$
\begin{aligned}
...
図\ref{fig:1} はジョルダン標準形に変換する前の座標系を表している. 初期時点 $t=0$ で $x$ が取りうる範囲は赤線で表された直線である.
1次元の自由度が与えられているのは, $x_{0}^{1}$ が固定されている一方で, $x_{0}^{2}$ が未知であることを反映している.

\begin{figure}
\centering
\includegraphics[width=9cm, page=3]{16EDFig.pdf}

\caption{変換前の直交座標系. $x_{0}^{1}$ だけが所与であり, $(x_{0}^{1},x_{0}^{2})$は赤色実線上のどこかにある.}
\label{fig:1}
\end{figure}

図 \ref{fig:1-1} はジョルダン標準形に変換した後の座標系である. すなわち, $P=[P_{1}\ P_{2}]$
...
式 \eqref{eq:jordan} を確認せよ. 

\begin{figure}
\centering
\includegraphics[width=9cm, page=4]{16EDFig.pdf}

\caption{ジョルダン標準形に変換した後の斜交座標系. 座標系を変更しても赤色実線は動かない. }
\label{fig:1-1}
\end{figure}

式 \eqref{eq:yu} は不安定固有値に対応する固有空間 ($y_{u}$ 軸) に沿って後ろ向きに $y_{0}^{u}$
を見つける手続きである. 将来にわたって $u$ が決定していれば, $t=\infty$ から $t=0$ まで時間を逆向きに解いて
$y_{0}^{u}$ を決めることができる. この時点ではまだ $y_{0}^{s}$ が決定しないので, $(y_{0}^{s},y_{0}^{u})$
は $(0,y_{0}^{u})$ を通り $y^{s}$ 軸と平行な直線上にある. 図\ref{fig:2} の青色実線を参照せよ.
この直線は安定行列 $J_{u}^{-1}$ によって特徴づけられる時間反転システムの極限を集めたものである. 

直交座標の $(x_{0}^{1},x_{0}^{2})$ と斜交座標の$(y_{0}^{s},y_{0}^{u})$ が同じ点を表す場合に限り,
それらは式\eqref{eq:sys} の$t=0$ における値として適切に選ばれていると言ってよい. 一方を他方の座標系で表して一致することを確かめればよいので,
式\eqref{eq:7}, \eqref{eq:9} を解けばよいことが分かるだろう. もちろんこれは \eqref{eq:det0}
と同値である. 解の一意決定性のための条件は, 赤線と青線の交点が一意に存在するための条件に外ならない (図 \ref{fig:3}).
なお, $P_{1s}$ の正則性は赤色の実線と青色の実線が平行にならないことを保証している. 

\begin{figure}
\centering
\includegraphics[width=9cm, page=5]{16EDFig.pdf}

\caption{安定な後ろ向き (forward-looking) サブシステム $y_{t}^{u}=J_{u}^{-1}y_{t+1}^{u}-J_{u}^{-1}C_{u}u_{t}$
...
\begin{figure}
\centering
\includegraphics[width=9cm, page=6]{16EDFig.pdf}

\caption{青色と赤色の直線が交わる点で $(x_{0}^{1},x_{0}^{2})$ あるいは $(y_{0}^{s},y_{0}^{u})$
が定まる.}
\label{fig:3}
\end{figure}

$(y_{0}^{s},y_{0}^{u})$ あるいは $(x_{0}^{1},x_{0}^{2})$ が定まれば, 式\eqref{eq:sys}
を繰り返し適用することで解軌道を決定できる. 

最後に, 解が存在しないケースと不決定の場合について述べる. まず, 固有値が2つとも不安定固有値である場合, すなわち $n_{u}=n=2$,
$0=n_{s}<n_{1}=1$ のケースを考えよう. 図\ref{fig:5} で示されている通り, 後ろ向き (forward-looking)
...
(図 \ref{fig:6}). 青色領域と赤色直線が交わる点は1次元の自由度を持つ無限個の点の集まりであるから $x_{0}^{2}$
を定めることもできない. 

\begin{figure}
\centering
\includegraphics[width=9cm, page=7]{16EDFig.pdf}

\caption{$n_{s}<n_{1}$ のケース. バックワードサブシステムが過剰に初期値を定めてしまう. 一般に解は存在しない. }
\label{fig:5}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=9cm, page=8]{16EDFig.pdf}

\caption{$n_{s}>n_{1}$ のケース. バックワードサブシステムの情報が不足しており, 赤色直線上の任意の点から出発した経路が解の性質を満たす.}
\label{fig:6}
\end{figure}
...
$$
Ex_{t+1}=Ax_{t}+Bu_{t},\quad t=0,1,\dots,\label{eq:sing}
$$
...
\begin{align}
y_{t+1}^{fs} & =J_{fs}y_{t}^{fs}+C_{fs}u_{t}\label{eq:forward}\\
...
C_{b}
\end{bmatrix}u_{t}\label{eq:backward}\\
 & =:\hat{J}_{u}y_{t+1}^{u}-D_{u}u_{t}.
\end{align}
{[}16ED04{]} の場合と異なり, 式 \eqref{eq:backward} が不安定な有限固有値に対応するサブシステムを時間反転して付加していることに注意してほしい.
$(E,A)$ の有限不安定固有値と無限大固有値をいずれも不安定固有値とみなすならば, 前節の結果を応用すれば解軌道が求まることも想像に難くないだろう.
ただし, $N$ の非正則性に伴うデリケートな問題が生じることに注意をしておこう. システム\ref{eq:sys} では, 初期値を定めるために後ろ向きのシステムを作ったが,
あくまでも順方向に決定されるシステムであり, 再帰的な構造を見つけることができた. しかし, システム \ref{eq:sing}
には無限大固有値に対応する本質的に非因果的な成分が含まれるので, $\det E=0$ である場合, 一般には再帰的に (因果的に)
記述できるとは限らない. 

### 解の存在と一意性

まずは, 式 \eqref{eq:backward} を解いてみよう. 
\begin{align}
y_{t}^{fu} & =-\sum_{k=0}^{\infty}J_{fu}^{-k-1}C_{fu}u_{t+k},\label{eq:yfu}\\
y_{t}^{b} & =N\left(Ny_{t+2}^{b}-C_{b}u_{t+1}\right)-C_{b}u_{t}\nonumber \\
 & =N^{n_{b}}y_{t+n_{b}}^{b}-\sum_{k=0}^{n_{b}-1}N^{k}C_{b}u_{t+k}\nonumber \\
 & =-\sum_{k=0}^{n_{b}-1}N^{k}C_{b}u_{t+k}.\label{eq:yb}
\end{align}
...
\end{bmatrix}
\end{align}
すでに決定している変数には上線を付している. 第\ref{sec:2}節のケースと同様に式\eqref{eq:init}を $(y_{0}^{fs},x_{0}^{2})$
...
\end{bmatrix}\begin{bmatrix}\bar{x}_{0}^{1}\\
\bar{y}_{0}^{u}
\end{bmatrix}\label{eq:init1}
$$
が得られる. 式\eqref{eq:init} から \eqref{eq:init1} は $t=0$ について記述しているが,
$t=0$ 以外の時点でも同様に成り立つことに注意せよ. 

定理 \ref{thm:blkn} の拡張となる次の結果を得る. 
```{theorem}
システム \eqref{eq:sing} に関して, $n_{1}=n_{fs}$ かつ $\det V_{1fs}\neq0$
が成り立てば, $(y_{0}^{fs},x_{0}^{2})$ および解が一意的に定まる. 
```

\begin{problem}
式 \eqref{eq:init1} を用いて, 解の不存在および不決定性が起こる場合の議論を完成させなさい.
\end{problem}
%
...
$$
 に従うと仮定する.\footnote{@Klein2000 は確率的なモデルを扱っているが, ここでの取り扱いと実質的には変わらない. }
式 \eqref{eq:yfu}, \eqref{eq:yb} を次のように書き換えておく.\footnote{この変形により $y_{t}^{b}$ は有限期先の未来にしか依存しないという情報を捨てている. }
$$
...
\end{aligned}
$$
ただし$M$は$\mathrm{vec}(M)=\left[\left(\Phi^{\top}\otimes\hat{J}_{u}\right)-I\right]^{-1}\mathrm{vec}\left(D_{u}\right)$によって決まる行列. 
```
