...
\end{align}
$y_{t}^{u}$ は式\@ref(eq:bk-yu) によって決定していることに注意せよ. なお, 式\@ref(eq:recur)
...
図\@ref(bk-beforetransformation) はジョルダン標準形に変換する前の座標系を表している. 初期時点 $t=0$ で $x$ が取りうる範囲は赤線で表された直線である.
...
knitr::include_graphics("figures/10-blanchardkahn/10-blanchardkahn.001.jpeg")
```
...
knitr::include_graphics("figures/10-blanchardkahn/10-blanchardkahn.002.jpeg")
```
...
```{r bk-initmanifold, fig.cap="(ref:cap-bk-initmanifold)", echo=FALSE}
knitr::include_graphics("figures/10-blanchardkahn/10-blanchardkahn.003.jpeg")
```

(ref:cap-bk-initmanifold) 安定な後ろ向き (forward-looking) サブシステム $y_{t}^{u}=J_{u}^{-1}y_{t+1}^{u}-J_{u}^{-1}C_{u}u_{t}$
...
knitr::include_graphics("figures/10-blanchardkahn/10-blanchardkahn.005.jpeg")
```
...
knitr::include_graphics("figures/10-blanchardkahn/10-blanchardkahn.006.jpeg")
```
