...
これまでは縦横に数字を並べたものとして行列を定義したが, これから線形空間の間の線形写像を表現としたものとして定式化する. 数ベクトル空間
$\mathbb{F}^{n\times1}$ を超えてより抽象的な線形空間の理論を持ち出す理由は, さもなくばシステム表現の変更に際して「変化しない実体」を見失ってしまう恐れがあるからである.
対角化やジョルダン標準形の理論, システムの正準形を理解する上で重要な役割を果たす. 

## 線形空間

### 定義

$X$ を集合, $\mathbb{F}$ を体とする. 
```{definition}
...
$$
が定義されていて,\footnote{写像 $f$ の定義域 (domain) が $X$, 終域 (codomain) が $Y$ であるとき $f:X\to Y$
と書く. すなわち, 任意の $x\in X$ について$f(x)$ が $Y$ の元として1つ定まる, ということを $f:X\to Y$
で表す. 一方, $f:x\mapsto y$ という表現もしばしば用いられる. 矢印の形が少し違うだけだが, 用途がかなり違うので注意が必要である.
$f:x\mapsto y$ と書いたとき, $x$ は $f$ によって $y$ に写されるという意味になる. 従って,
$x$ も $y$ も既知であるようなときにその対応関係を$f$ によって表すという意味で使われる. 上の場合は, $+$
という記号を和の写像を表すのに使うという程度の意味である. } 以下の諸条件を満たすことをいう. 線形空間の元をベクトルと呼ぶ. 
...
$$
は成分ごとの和とスカラー倍によってベクトル空間となる. 
```


```{example}
$m\times n$ 行列全体の集合は線形空間をなす. 資料 \href{http://16ED02\#page=4}{[16ED02]}
の第1節を参照.
```


```{example}
高々 $n$ 次の多項式全体の集合 $P^{n}[z]$ も通常の和と定数倍に関してベクトル空間となる. 
```
...
```{example}
有限期待値を持つ実確率変数の空間は$\mathbb{R}$ 上のベクトル空間となる.\footnote{$\mathbb{E}[X+Y]=\mathbb{E}X+\mathbb{E}Y$ といった式が成り立つ.} 
```
...
```{block2, type='fact'}
$Y$ が$X$の線形部分空間であるための必要十分条件は, $Y$ が和とスカラー倍について閉じていることである. すなわち,
任意の $y_{1},y_{2}\in Y$, $\alpha\in\mathbb{F}$について, $y_{1}+y_{2}\in Y$
と $\alpha y_{1}\in Y$が成り立つことを確認すればよい. 
```
...
によって張られる線形部分空間といい, $\mathrm{span}\{x_{1},\dots,x_{n}\}$ と書く. 
$$
\mathrm{span}\{x_{1},\dots,x_{n}\}=\left\{ \alpha_{1}x_{1}+\cdots+\alpha_{n}x_{n}\ \mid\ \alpha_{1},\dots,\alpha_{n}\in\mathbb{F}\right\} 
$$
である. なお, $\alpha_{1}x_{1}+\cdots+\alpha_{n}x_{n}$ の形のベクトルは $\{x_{1},\dots,x_{n}\}$
の1次結合であるという.
```


2つの部分空間 $Y$, $W$ があったとき, $Y\cap W$ はまた線形部分空間になる. 一方, $Y\cup W$
は一般には線形部分空間にならない. 
```{definition}
$X$ を$\mathbb{F}$ 上の線形空間, $Y,W$ を$X$ の線形部分空間とする. 部分空間の和 $Y+W$
は$Y$ と$W$ の元をすべて含む最小の線形部分空間と定義される. すなわち
\begin{equation}
Y+W=\{\alpha y+\beta w\ \mid\ y\in Y,\ w\in W,\ \alpha,\beta\in\mathbb{F}\}.\label{eq:sum}
\end{equation}
```

上の定義 (\ref{eq:sum}) は和とスカラー倍について閉じているように作られているので部分空間となる. 
```{definition}
$M$, $N$ を$X$ の線型部分空間, $M\cap N=\{0\}$ とする. 和 $M+N$ を特に直和といい,
$M\oplus N$ と記す. 
```
...
```{proof}
$M$の非ゼロ元と$N$ の非ゼロ元が常に1次独立であることを確認すればよい. $u,u'\in M$ と $v,v'\in N$
がともに $x=u+v$ と $x=u'+v'$ を満たすとしよう. $u-u'=v'-v\in M\cap N=\{0\}$
より $u=u'$ と $v=v'$ が分かる.
```
...
$$
\alpha_{1}x_{1}+\cdots+\alpha_{d}x_{d}=0\Longrightarrow\alpha_{1}=\cdots=\alpha_{d}=0
$$
...
$$
と書ける. 逆も成り立つことが簡単に示せるので, 確認しておくこと. 
...
```{theorem}
\label{thm:dim}有限次元ベクトル空間の基底の数は基底の選び方にかかわらず一定である.
```
...
\begin{equation}
\sum_{i=1}^{m}\beta_{i}\alpha_{ij}=0,\quad j=1,\dots,n\label{eq:1}
\end{equation}
...
\end{bmatrix}Q^{-1}=0
$$
を得る.\footnote{資料 {[}16ED02{]} を参照. } さらに右から$Q$をかければ
$$
\begin{bmatrix}\beta_{1} & \cdots & \beta_{m}\end{bmatrix}P^{-1}\begin{bmatrix}I_{n\times n}\\
O_{(m-n)\times n}
...
$$
より, $\beta_{1},\dots,\beta_{m}$ のすべてがゼロでなくても (\ref{eq:1}) を成り立たせることができることが分かる
...
onto mapping)であるという.\footnote{$f:X\to Y$ は通常, ``$f$ maps $X$ into $Y$'' と読むが, $f$ が全射のときは ``$f$
maps $X$ onto $Y$'' と読む. }
...
$$
と表現できる.\footnote{$P=[p_{ij}]_{ij}$ とすれば, $\tilde{v}_{i}=\sum_{i=1}^{n}p_{ij}v_{i}$
...
\begin{bmatrix}f(\tilde{v}_{1}) & \cdots & f(\tilde{v}_{n})\end{bmatrix} & =\begin{bmatrix}f(v_{1}) & \cdots & f(v_{n})\end{bmatrix}P\nonumber \\
 ...
 & =\begin{bmatrix}\tilde{w}_{1} & \cdots & \tilde{w}_{m}\end{bmatrix}Q^{-1}AP\label{eq:change}
\end{align}
