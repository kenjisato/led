...
仮に $\lambda$ が複素根であるとすれば，$\bar \lambda$ も根であるから，$\bar \lambda = \beta^{-1} \lambda^{-1}$ 
が成り立たなければ根が4つになってしまうので不合理である。$\bar \lambda = \beta^{-1} \lambda^{-1}$ として計算を進めると方程式 \@ref(eq:lineuler) 
